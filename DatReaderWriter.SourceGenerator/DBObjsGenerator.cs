    ...
    public class DBObjsGenerator : BaseGenerator {
        ...
        public override void Generate(SourceProductionContext spc, XMLDefParser parser) {
            foreach (var kv in parser.ACDBObjs) {
                // if (kv.Value.Children.Count == 0) continue; // Allow empty DBObjs (e.g. RenderMaterial)
                if (kv.Key == "DBProperties" || kv.Key == "MasterProperty" || kv.Key == "LandBlock" ||
                    kv.Key == "ActionMap" || kv.Key == "Iteration") {
                    continue;
                }
                ...
                using (writer.IndentScope()) {
                    ...
                    using (writer.IndentScope()) {
                        ...
                        // HeaderFlags
                        var flags = string.IsNullOrEmpty(dbObj.DBObjHeaderFlags)
                            ? "DBObjHeaderFlags.None"
                            : dbObj.DBObjHeaderFlags;
